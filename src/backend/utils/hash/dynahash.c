...
hash_create(const char *tabname, long nelem, const HASHCTL *info, int flags)
{
	...
	if (!hashp->hctl)
	{
		Size		size = hash_get_init_size(info, flags, init_size, nelem);
 ...
 *
 * init_size is the total number of elements actually allocated during hash
 * table creation, it could be zero for non-shared hash tables depending on
 * whether we decide to pre-allocate elements or not(determined by the caller).
 * 
 * For a shared hash table init_size and nelem are always equal.
 */
Size
hash_get_init_size(const HASHCTL *info, int flags, long init_size, long nelem)
{
	int			nbuckets;
	int			nsegs;
	int			num_partitions;
	...
	/* init_size equal to 0 indicates no elements are to be pre-allocated */
	if (init_size != 0)
		init_size = nelem;
